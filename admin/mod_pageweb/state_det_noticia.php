...
if(isset($_POST['id']) && trim($_POST['id'])<>""){
	...
	if(fncVerificarID($reg_id)==1){
		...
		$consulta = "SELECT id_detalle FROM news_detalle WHERE id_detalle='".$reg_id."'";
		$rs = $_oControl->consulta($consulta);
		$total = $_oControl->nroregistros;
		if($total<>0){
			$reg_es = fncSeguridad($_POST["es"]);
			if(trim($reg_es)<>"" && ($reg_es=="A" || $reg_es=="I")){
				$editar = $_oControl->mantto("UPDATE news_detalle SET estado='".$reg_es."' WHERE id_detalle='".$reg_id."'");
